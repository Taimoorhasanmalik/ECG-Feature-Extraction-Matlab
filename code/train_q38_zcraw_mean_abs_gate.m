%% Train Normal vs Abnormal using Q3.8-quantized raw-zero-crossing window features, recall-focused
...
% Features:
%   1. mean_abs: mean absolute ECG amplitude inside the demeaned window
%   2. window_mean: mean ECG amplitude inside the quantized window
%   3. zero_crossings: zero-crossing rate of the demeaned ECG window
%   4. line_length: normalized signal path length inside the window
%   5. threshold_crossing_count: adaptive threshold crossing count
...
modelStem = "stage1_q38_zcraw_mean_abs_recall95_lsvm";
if cleanTrainingEnabled
    modelStem = "stage1_q38_zcraw_mean_abs_recall95_clean_lsvm";
end
...
allFeatureNames = {'mean_abs', 'window_mean', 'zero_crossings', 'line_length', ...
...
if useGPU
    ...
    try
        for batchStart = 1:batchSize:size(X, 1)
            batchEnd = min(size(X, 1), batchStart + batchSize - 1);
            [~, scoresGpu] = predict(model, gpuArray(single(X(batchStart:batchEnd, :))));
            scores(batchStart:batchEnd, :) = gather(scoresGpu);
        end
        return;
    catch ME
        error('GPU predict failed and CPU fallback is disabled because useGPURequested=true. Reason: %s', ...
            ME.message);
    end
end
...
for w = 1:numWindows
    ...
    rawWindowSignal = ecg(startIdx:endIdx);
    ...
    windowMean = mean(rawWindowSignal, 'omitnan');
    windowSignal = rawWindowSignal - windowMean;
    diffSignal = diff(windowSignal);

    meanAbs = mean(abs(windowSignal), 'omitnan');
    zeroCrossings = count_zero_crossings(windowSignal) / windowSeconds;
    ...
    X_file(w, :) = [meanAbs, windowMean, zeroCrossings, lineLength, thresholdCrossingCount, ...
