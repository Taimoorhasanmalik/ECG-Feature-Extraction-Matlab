...
fprintf('Abnormal augmentation: SMOTE-style interpolation | factor %.2f | seed %g\n', ...
...
if useGPURequested
    if ~strcmpi(svmKernel, 'linear')
        error('GPU training is requested, but only the linear SVM GPU path is supported.');
    end
    if svmImplementation ~= "fitcsvm"
        error('GPU training is requested, but fitcsvm is unavailable.');
    end
    if ~can_use_gpu_arrays()
        error('GPU training is requested, but MATLAB cannot access a GPU. Check Parallel Computing Toolbox and gpuDevice.');
    end
    useGPUTraining = true;
    fprintf('GPU training requested: available, will require gpuArray fitcsvm.\n');
else
...
S = load(modelFile);
test_scores = predict_scores_optional_gpu(S.normal_abnormal_svm_model, test_X, S.finalUsedGPU);
test_pred = double(test_scores(:, 2) >= S.decisionThreshold);

minConsecutiveAbnormalWindows = 3;
two_window_pred = false(size(test_pred));
singleton_pred = false(size(test_pred));
testRecordValues = unique(test_record_ids(:))';
for recordValue = testRecordValues
    recordRows = find(test_record_ids == recordValue);
    recordPred = logical(test_pred(recordRows));
    runStart = [];
    for idx = 1:numel(recordRows)
        if recordPred(idx) && isempty(runStart)
            runStart = idx;
        end
        isRunEnd = ~recordPred(idx) || idx == numel(recordRows);
        if isRunEnd && ~isempty(runStart)
            if recordPred(idx)
                runEnd = idx;
            else
                runEnd = idx - 1;
            end
            runRows = recordRows(runStart:runEnd);
            if numel(runRows) >= minConsecutiveAbnormalWindows
                two_window_pred(runRows) = true;
            else
                singleton_pred(runRows) = true;
            end
            runStart = [];
        end
    end
...
two_window_pred = double(two_window_pred);
singleton_pred = double(singleton_pred);
twoWindowMetrics = binary_metrics(test_Y, two_window_pred, test_scores(:, 2));

singletonCount = sum(singleton_pred == 1);
singletonTrueAbnormal = sum(singleton_pred == 1 & test_Y == 1);
singletonFalseNormal = sum(singleton_pred == 1 & test_Y == 0);
strictTP = twoWindowMetrics.TP;
strictFP = twoWindowMetrics.FP + singletonCount;
strictPrecision = strictTP / (strictTP + strictFP + eps);

fprintf('\n%d-window invocation rule on untouched holdout set:\n', ...
    minConsecutiveAbnormalWindows);
fprintf('Total windows: %d\n', length(test_Y));
fprintf('Original abnormal predictions/invocations: %d\n', sum(test_pred == 1));
fprintf('Original false invocations: %d (%.2f%% of all windows)\n', ...
    S.holdoutTestMetrics.FP, 100 * S.holdoutTestMetrics.FP / length(test_Y));
fprintf('Singleton abnormal predictions suppressed: %d\n', singletonCount);
fprintf('  Singleton true-abnormal windows: %d\n', singletonTrueAbnormal);
fprintf('  Singleton normal windows: %d\n', singletonFalseNormal);
fprintf('%d-window invocations: %d\n', ...
    minConsecutiveAbnormalWindows, sum(two_window_pred == 1));
fprintf('Wrong %d-window invocations: %d (%.2f%% of all windows, %.2f%% of invocations)\n', ...
    minConsecutiveAbnormalWindows, ...
    twoWindowMetrics.FP, 100 * twoWindowMetrics.FP / length(test_Y), ...
    100 * twoWindowMetrics.FP / max(sum(two_window_pred == 1), 1));
fprintf('%d-window precision: %.4f\n', ...
    minConsecutiveAbnormalWindows, twoWindowMetrics.precision);
fprintf('%d-window recall: %.4f\n', ...
    minConsecutiveAbnormalWindows, twoWindowMetrics.recall);
fprintf('%d-window confusion [TN FP; FN TP]:\n', ...
    minConsecutiveAbnormalWindows);
disp([twoWindowMetrics.TN twoWindowMetrics.FP; twoWindowMetrics.FN twoWindowMetrics.TP]);
fprintf('Precision if every singleton abnormal prediction is counted as false: %.4f\n', ...
    strictPrecision);
...
quantizedEcg = round(ecg * scale) / scale;
