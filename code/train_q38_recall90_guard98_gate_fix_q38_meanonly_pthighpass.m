%% Train Normal vs Abnormal using causal Pan-Tompkins high-pass and fix Q3.8 mean-only features
...
% to signed Q3.8 using fix(), applies a causal 5 Hz Pan-Tompkins-style
% high-pass, then quantizes mean values only after sum/windowLength division
...
modelStem = "stage1_normal_vs_abnormal_window_quantized_q3_8_fix_meanonly_pthighpass_recall90_guard98_robust_lsvm";
if cleanTrainingEnabled
    modelStem = "stage1_normal_vs_abnormal_window_quantized_q3_8_fix_meanonly_pthighpass_recall90_guard98_clean_lsvm";
end
...
quantizationStageMode = 'signal_pthighpass_and_window_mean_only';
...
preprocessingMode = 'causal_pantompkins_highpass';
preprocessingHighpassHz = 5;
preprocessingHighpassOrder = 3;
...
fprintf('Preprocessing: causal Pan-Tompkins-style %.1f Hz high-pass, order %d, no whole-record median removal.\n', ...
    preprocessingHighpassHz, preprocessingHighpassOrder);
...
for fileIdx = 1:length(recordFiles)
    ...
    ecg = apply_causal_pantompkins_highpass(ecg, Fs, preprocessingHighpassHz, ...
        preprocessingHighpassOrder);
    ecg = quantize_signed_q_format(ecg, quantizationIntegerBits, quantizationFractionBits);
...
function filteredEcg = apply_causal_pantompkins_highpass(ecg, Fs, highpassHz, filterOrder)
ecg = double(ecg(:));
if isempty(ecg)
    filteredEcg = ecg;
    return;
...
nyquistHz = Fs / 2;
if highpassHz <= 0 || highpassHz >= nyquistHz
    error('Invalid Pan-Tompkins high-pass limit %.3g Hz for Fs %.3g Hz.', ...
        highpassHz, Fs);
end

[b, a] = butter(filterOrder, highpassHz / nyquistHz, 'high');
ecg(~isfinite(ecg)) = 0;
...
function [X_file, Y_file] = extract_window_features(ecg, sampleLabels, Fs, windowSeconds, stepSeconds, minClassFraction, thresholdFraction, artifactMask, artifactMaxWindowFraction, quantizationIntegerBits, quantizationFractionBits)
ecg = ecg(:);
