...
W_all = [];  % rows: [R Q S T RR QS mean_ecg]
...
for recIdx = 1:numel(records)
    ...
    % Window-level aggregation: mean(R/Q/S/T/RR/QS within window) + mean(ecg window)
    ecg_vec = ecg(:);
    winSamples = round(windowSec * Fs);
    hopSamples = round(hopSec * Fs);

    if hopSamples > 0 && numel(ecg_vec) >= winSamples
        winStarts = 1:hopSamples:(numel(ecg_vec) - winSamples + 1);

        R_ind_w = R_ind(1:obs);
        R_vals_w = R_vals(1:obs);
        Q_vals_w = Q_vals(1:obs);
        S_vals_w = S_vals(1:obs);
        T_vals_w = T_vals(1:obs);
        RR_w = RR_int(1:obs);
        QS_w = QS_int(1:obs);
        ...
        for w = 1:numel(winStarts)
            ...
            inWin = (R_ind_w >= s) & (R_ind_w <= e);
            if ~any(inWin)
                continue;
            end

            mean_ecg = mean(ecg_vec(s:e), 'omitnan');

            W_all = [W_all; [ ...
                mean(R_vals_w(inWin), 'omitnan'), ...
                mean(Q_vals_w(inWin), 'omitnan'), ...
                mean(S_vals_w(inWin), 'omitnan'), ...
                mean(T_vals_w(inWin), 'omitnan'), ...
                mean(RR_w(inWin), 'omitnan'), ...
                mean(QS_w(inWin), 'omitnan'), ...
                mean_ecg ...
            ]]; %#ok<AGROW>
        end
    end
...
if ~isempty(W_all)
    C = corr(W_all, 'Rows', 'complete');
    corrWithMean = C(1:6, 7);
    labels = {'R', 'Q', 'S', 'T', 'RR', 'QS'};
    [~, order] = sort(abs(corrWithMean), 'descend');

    fprintf('\nCorrelation with mean ECG (window-level):\n');
    for k = 1:numel(order)
        idx = order(k);
        fprintf('corr(%s, mean_ecg) = %.4f\n', labels{idx}, corrWithMean(idx));
    end
end
